...
// the scene that the widget is embedded in. 3D widgets work in the context环境
...
#include "vtkCommand.h"//新
#include "vtkBoxWidget.h"//新
#include "vtkTransform.h"//新
#include "vtkInteractorStyleTrackballCamera.h"

//
// Similar to Cone2.cxx, we define a callback回收信号 for interaction.
//
...
int main()
{
  ...
  //以下部分为有别于cone5的boxwiget部分 
  // Here we use a vtkBoxWidget to transform the underlying coneActor (by
  // manipulating its transformation matrix变换矩阵). Many other types of widgets
  // are available for use, see the documentation for more details.
  //
  // The SetInteractor method is how 3D widgets are associated with the render
  // window interactor. Internally内在的, SetInteractor sets up a bunch of callbacks
  // using the Command/Observer mechanism (AddObserver()). The place factor
  // controls the initial size of the widget with respect to关于 the bounding box
  // of the input to the widget.
  vtkBoxWidget *boxWidget = vtkBoxWidget::New();
  boxWidget->SetInteractor(iren);
  boxWidget->SetPlaceFactor(1.25);//控制初始的框与cone之间的大小倍数关系
